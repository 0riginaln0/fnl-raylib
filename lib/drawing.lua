...
local ffi = dll.ffi
local rl = dll.rl
ffi.cdef("\13\nvoid ClearBackground(Color color);                          // Set background color (framebuffer clear color)\13\n\13\nvoid BeginDrawing(void);                                    // Setup canvas (framebuffer) to start drawing\13\n\13\nvoid EndDrawing(void);                                      // End canvas drawing and swap buffers (double buffering)\13\n\13\nvoid BeginMode2D(Camera2D camera);                          // Begin 2D mode with custom camera (2D)\13\n\13\nvoid EndMode2D(void);                                       // Ends 2D mode with custom camera\13\n\13\nvoid BeginMode3D(Camera3D camera);                          // Begin 3D mode with custom camera (3D)\13\n\13\nvoid EndMode3D(void);                                       // Ends 3D mode and returns to default 2D orthographic mode\13\n\13\nvoid BeginTextureMode(RenderTexture2D target);              // Begin drawing to render texture\13\n\13\nvoid EndTextureMode(void);                                  // Ends drawing to render texture\13\n\13\nvoid BeginShaderMode(Shader shader);                        // Begin custom shader drawing\13\n\13\nvoid EndShaderMode(void);                                   // End custom shader drawing (use default shader)\13\n\13\nvoid BeginBlendMode(int mode);                              // Begin blending mode (alpha, additive, multiplied, subtract, custom)\13\n\13\nvoid EndBlendMode(void);                                    // End blending mode (reset to default: alpha blending)\13\n\13\nvoid BeginScissorMode(int x, int y, int width, int height); // Begin scissor mode (define screen area for following drawing)\13\n\13\nvoid EndScissorMode(void);                                  // End scissor mode\13\n\13\nvoid BeginVrStereoMode(VrStereoConfig config);              // Begin stereo rendering (requires VR simulator)\13\n\13\nvoid EndVrStereoMode(void);                                 // End stereo rendering (requires VR simulator)\13\n\13\nVrStereoConfig LoadVrStereoConfig(VrDeviceInfo device);     // Load VR stereo config for VR simulator device parameters\13\n\13\nvoid UnloadVrStereoConfig(VrStereoConfig config);           // Unload VR stereo config\13\n\13\nShader LoadShader(const char *vsFileName, const char *fsFileName);   // Load shader from files and bind default locations\13\n\13\nShader LoadShaderFromMemory(const char *vsCode, const char *fsCode); // Load shader from code strings and bind default locations\13\n\13\nbool IsShaderValid(Shader shader);                                   // Check if a shader is valid (loaded on GPU)\13\n\13\nint GetShaderLocation(Shader shader, const char *uniformName);       // Get shader uniform location\13\n\13\nint GetShaderLocationAttrib(Shader shader, const char *attribName);  // Get shader attribute location\13\n\13\nvoid SetShaderValue(Shader shader, int locIndex, const void *value, int uniformType);               // Set shader uniform value\13\n\13\nvoid SetShaderValueV(Shader shader, int locIndex, const void *value, int uniformType, int count);   // Set shader uniform value vector\13\n\13\nvoid SetShaderValueMatrix(Shader shader, int locIndex, Matrix mat);         // Set shader uniform value (matrix 4x4)\13\n\13\nvoid SetShaderValueTexture(Shader shader, int locIndex, Texture2D texture); // Set shader uniform value for texture (sampler2d)\13\n\13\nvoid UnloadShader(Shader shader);                                    // Unload shader from GPU memory (VRAM)\13\n\13\nRay GetScreenToWorldRay(Vector2 position, Camera camera);         // Get a ray trace from screen position (i.e mouse)\13\n\13\nRay GetScreenToWorldRayEx(Vector2 position, Camera camera, int width, int height); // Get a ray trace from screen position (i.e mouse) in a viewport\13\n\13\nVector2 GetWorldToScreen(Vector3 position, Camera camera);        // Get the screen space position for a 3d world space position\13\n\13\nVector2 GetWorldToScreenEx(Vector3 position, Camera camera, int width, int height); // Get size position for a 3d world space position\13\n\13\nVector2 GetWorldToScreen2D(Vector2 position, Camera2D camera);    // Get the screen space position for a 2d camera world space position\13\n\13\nVector2 GetScreenToWorld2D(Vector2 position, Camera2D camera);    // Get the world space position for a 2d camera screen space position\13\n\13\nMatrix GetCameraMatrix(Camera camera);                            // Get camera transform matrix (view matrix)\13\n\13\nMatrix GetCameraMatrix2D(Camera2D camera);                        // Get camera 2d transform matrix\13\n\13\nvoid SetTargetFPS(int fps);                                 // Set target FPS (maximum)\13\n\13\nfloat GetFrameTime(void);                                   // Get time in seconds for last frame drawn (delta time)\13\n\13\ndouble GetTime(void);                                       // Get elapsed time in seconds since InitWindow()\13\n\13\nint GetFPS(void);                                           // Get current FPS\13\n\13\nvoid SwapScreenBuffer(void);                                // Swap back buffer with front buffer (screen drawing)\13\n\13\nvoid PollInputEvents(void);                                 // Register all input events\13\n\13\nvoid WaitTime(double seconds);                              // Wait for some time (halt program execution)\13\n\13\nvoid SetRandomSeed(unsigned int seed);                      // Set the seed for the random number generator\13\n\13\nint GetRandomValue(int min, int max);                       // Get a random value between min and max (both included)\13\n\13\nint *LoadRandomSequence(unsigned int count, int min, int max); // Load random values sequence, no values repeated\13\n\13\nvoid UnloadRandomSequence(int *sequence);                   // Unload random values sequence\13\n\13\nvoid TakeScreenshot(const char *fileName);                  // Takes a screenshot of current screen (filename extension defines format)\13\n\13\nvoid SetConfigFlags(unsigned int flags);                    // Setup init configuration flags (view FLAGS)\13\n\13\nvoid OpenURL(const char *url);                              // Open URL with default system browser (if available)\13\n\13\nvoid TraceLog(int logLevel, const char *text, ...);         // Show trace log messages (LOG_DEBUG, LOG_INFO, LOG_WARNING, LOG_ERROR...)\13\n\13\nvoid SetTraceLogLevel(int logLevel);                        // Set the current threshold (minimum) log level\13\n\13\nvoid *MemAlloc(unsigned int size);                          // Internal memory allocator\13\n\13\nvoid *MemRealloc(void *ptr, unsigned int size);             // Internal memory reallocator\13\n\13\nvoid MemFree(void *ptr);                                    // Internal memory free\13\ntypedef void (*TraceLogCallback)(int logLevel, const char *text, va_list args);  // Logging: Redirect trace log messages\13\n\13\nvoid SetTraceLogCallback(TraceLogCallback callback);         // Set custom trace log\13\n\13\ntypedef unsigned char *(*LoadFileDataCallback)(const char *fileName, int *dataSize);    // FileIO: Load binary data\13\nvoid SetLoadFileDataCallback(LoadFileDataCallback callback); // Set custom file binary data loader\13\n\13\ntypedef bool (*SaveFileDataCallback)(const char *fileName, void *data, int dataSize);   // FileIO: Save binary data\13\nvoid SetSaveFileDataCallback(SaveFileDataCallback callback); // Set custom file binary data saver\13\n\13\ntypedef char *(*LoadFileTextCallback)(const char *fileName);            // FileIO: Load text data\13\nvoid SetLoadFileTextCallback(LoadFileTextCallback callback); // Set custom file text data loader\13\n\13\ntypedef bool (*SaveFileTextCallback)(const char *fileName, char *text); // FileIO: Save text data\13\nvoid SetSaveFileTextCallback(SaveFileTextCallback callback); // Set custom file text data saver\13\n\13\nunsigned char *LoadFileData(const char *fileName, int *dataSize); // Load file data as byte array (read)\13\n\13\nvoid UnloadFileData(unsigned char *data);                   // Unload file data allocated by LoadFileData()\13\n\13\nbool SaveFileData(const char *fileName, void *data, int dataSize); // Save data to file from byte array (write), returns true on success\13\n\13\nbool ExportDataAsCode(const unsigned char *data, int dataSize, const char *fileName); // Export data to code (.h), returns true on success\13\n\13\nchar *LoadFileText(const char *fileName);                   // Load text data from file (read), returns a '\0' terminated string\13\n\13\nvoid UnloadFileText(char *text);                            // Unload file text data allocated by LoadFileText()\13\n\13\nbool SaveFileText(const char *fileName, char *text);        // Save text data to file (write), string must be '\0' terminated, returns true on success\13\n\13\nbool FileExists(const char *fileName);                      // Check if file exists\13\n\13\nbool DirectoryExists(const char *dirPath);                  // Check if a directory path exists\13\n\13\nbool IsFileExtension(const char *fileName, const char *ext); // Check file extension (including point: .png, .wav)\13\n\13\nint GetFileLength(const char *fileName);                    // Get file length in bytes (NOTE: GetFileSize() conflicts with windows.h)\13\n\13\nconst char *GetFileExtension(const char *fileName);         // Get pointer to extension for a filename string (includes dot: '.png')\13\n\13\nconst char *GetFileName(const char *filePath);              // Get pointer to filename for a path string\13\n\13\nconst char *GetFileNameWithoutExt(const char *filePath);    // Get filename string without extension (uses static string)\13\n\13\nconst char *GetDirectoryPath(const char *filePath);         // Get full path for a given fileName with path (uses static string)\13\n\13\nconst char *GetPrevDirectoryPath(const char *dirPath);      // Get previous directory path for a given path (uses static string)\13\n\13\nconst char *GetWorkingDirectory(void);                      // Get current working directory (uses static string)\13\n\13\nconst char *GetApplicationDirectory(void);                  // Get the directory of the running application (uses static string)\13\n\13\nint MakeDirectory(const char *dirPath);                     // Create directories (including full path requested), returns 0 on success\13\n\13\nbool ChangeDirectory(const char *dir);                      // Change working directory, return true on success\13\n\13\nbool IsPathFile(const char *path);                          // Check if a given path is a file or a directory\13\n\13\nbool IsFileNameValid(const char *fileName);                 // Check if fileName is valid for the platform/OS\13\n\13\nFilePathList LoadDirectoryFiles(const char *dirPath);       // Load directory filepaths\13\n\13\nFilePathList LoadDirectoryFilesEx(const char *basePath, const char *filter, bool scanSubdirs); // Load directory filepaths with extension filtering and recursive directory scan. Use 'DIR' in the filter string to include directories in the result\13\n\13\nvoid UnloadDirectoryFiles(FilePathList files);              // Unload filepaths\13\n\13\nbool IsFileDropped(void);                                   // Check if a file has been dropped into window\13\n\13\nFilePathList LoadDroppedFiles(void);                        // Load dropped filepaths\13\n\13\nvoid UnloadDroppedFiles(FilePathList files);                // Unload dropped filepaths\13\n\13\nlong GetFileModTime(const char *fileName);                  // Get file modification time (last write time)\13\n\13\nunsigned char *CompressData(const unsigned char *data, int dataSize, int *compDataSize);        // Compress data (DEFLATE algorithm), memory must be MemFree()\13\n\13\nunsigned char *DecompressData(const unsigned char *compData, int compDataSize, int *dataSize);  // Decompress data (DEFLATE algorithm), memory must be MemFree()\13\n\13\nchar *EncodeDataBase64(const unsigned char *data, int dataSize, int *outputSize);               // Encode data to Base64 string, memory must be MemFree()\13\n\13\nunsigned char *DecodeDataBase64(const unsigned char *data, int *outputSize);                    // Decode Base64 string data, memory must be MemFree()\13\n\13\nunsigned int ComputeCRC32(unsigned char *data, int dataSize);     // Compute CRC32 hash code\13\n\13\nunsigned int *ComputeMD5(unsigned char *data, int dataSize);      // Compute MD5 hash code, returns static int[4] (16 bytes)\13\n\13\nunsigned int *ComputeSHA1(unsigned char *data, int dataSize);      // Compute SHA1 hash code, returns static int[5] (20 bytes)\13\n\13\nAutomationEventList LoadAutomationEventList(const char *fileName);                // Load automation events list from file, NULL for empty list, capacity = MAX_AUTOMATION_EVENTS\13\n\13\nvoid UnloadAutomationEventList(AutomationEventList list);                         // Unload automation events list from file\13\n\13\nbool ExportAutomationEventList(AutomationEventList list, const char *fileName);   // Export automation events list as text file\13\n\13\nvoid SetAutomationEventList(AutomationEventList *list);                           // Set automation event list to record to\13\n\13\nvoid SetAutomationEventBaseFrame(int frame);                                      // Set automation event internal base frame to start recording\13\n\13\nvoid StartAutomationEventRecording(void);                                         // Start recording automation events (AutomationEventList must be set)\13\n\13\nvoid StopAutomationEventRecording(void);                                          // Stop recording automation events\13\n\13\nvoid PlayAutomationEvent(AutomationEvent event);                                  // Play a recorded automation event\13\n\13\n")
