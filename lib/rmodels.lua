...
local ffi = dll.ffi
local rl = dll.rl
ffi.cdef("\13\nvoid DrawLine3D(Vector3 startPos, Vector3 endPos, Color color);                                    // Draw a line in 3D world space\13\n\13\nvoid DrawPoint3D(Vector3 position, Color color);                                                   // Draw a point in 3D space, actually a small line\13\n\13\nvoid DrawCircle3D(Vector3 center, float radius, Vector3 rotationAxis, float rotationAngle, Color color); // Draw a circle in 3D world space\13\n\13\nvoid DrawTriangle3D(Vector3 v1, Vector3 v2, Vector3 v3, Color color);                              // Draw a color-filled triangle (vertex in counter-clockwise order!)\13\n\13\nvoid DrawTriangleStrip3D(const Vector3 *points, int pointCount, Color color);                      // Draw a triangle strip defined by points\13\n\13\nvoid DrawCube(Vector3 position, float width, float height, float length, Color color);             // Draw cube\13\n\13\nvoid DrawCubeV(Vector3 position, Vector3 size, Color color);                                       // Draw cube (Vector version)\13\n\13\nvoid DrawCubeWires(Vector3 position, float width, float height, float length, Color color);        // Draw cube wires\13\n\13\nvoid DrawCubeWiresV(Vector3 position, Vector3 size, Color color);                                  // Draw cube wires (Vector version)\13\n\13\nvoid DrawSphere(Vector3 centerPos, float radius, Color color);                                     // Draw sphere\13\n\13\nvoid DrawSphereEx(Vector3 centerPos, float radius, int rings, int slices, Color color);            // Draw sphere with extended parameters\13\n\13\nvoid DrawSphereWires(Vector3 centerPos, float radius, int rings, int slices, Color color);         // Draw sphere wires\13\n\13\nvoid DrawCylinder(Vector3 position, float radiusTop, float radiusBottom, float height, int slices, Color color); // Draw a cylinder/cone\13\n\13\nvoid DrawCylinderEx(Vector3 startPos, Vector3 endPos, float startRadius, float endRadius, int sides, Color color); // Draw a cylinder with base at startPos and top at endPos\13\n\13\nvoid DrawCylinderWires(Vector3 position, float radiusTop, float radiusBottom, float height, int slices, Color color); // Draw a cylinder/cone wires\13\n\13\nvoid DrawCylinderWiresEx(Vector3 startPos, Vector3 endPos, float startRadius, float endRadius, int sides, Color color); // Draw a cylinder wires with base at startPos and top at endPos\13\n\13\nvoid DrawCapsule(Vector3 startPos, Vector3 endPos, float radius, int slices, int rings, Color color); // Draw a capsule with the center of its sphere caps at startPos and endPos\13\n\13\nvoid DrawCapsuleWires(Vector3 startPos, Vector3 endPos, float radius, int slices, int rings, Color color); // Draw capsule wireframe with the center of its sphere caps at startPos and endPos\13\n\13\nvoid DrawPlane(Vector3 centerPos, Vector2 size, Color color);                                      // Draw a plane XZ\13\n\13\nvoid DrawRay(Ray ray, Color color);                                                                // Draw a ray line\13\n\13\nvoid DrawGrid(int slices, float spacing);                                                          // Draw a grid (centered at (0, 0, 0))\13\n\13\nModel LoadModel(const char *fileName);                                                // Load model from files (meshes and materials)\13\n\13\nModel LoadModelFromMesh(Mesh mesh);                                                   // Load model from generated mesh (default material)\13\n\13\nbool IsModelValid(Model model);                                                       // Check if a model is valid (loaded in GPU, VAO/VBOs)\13\n\13\nvoid UnloadModel(Model model);                                                        // Unload model (including meshes) from memory (RAM and/or VRAM)\13\n\13\nBoundingBox GetModelBoundingBox(Model model);                                         // Compute model bounding box limits (considers all meshes)\13\n\13\nvoid DrawModel(Model model, Vector3 position, float scale, Color tint);               // Draw a model (with texture if set)\13\n\13\nvoid DrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint); // Draw a model with extended parameters\13\n\13\nvoid DrawModelWires(Model model, Vector3 position, float scale, Color tint);          // Draw a model wires (with texture if set)\13\n\13\nvoid DrawModelWiresEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint); // Draw a model wires (with texture if set) with extended parameters\13\n\13\nvoid DrawModelPoints(Model model, Vector3 position, float scale, Color tint); // Draw a model as points\13\n\13\nvoid DrawModelPointsEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint); // Draw a model as points with extended parameters\13\n\13\nvoid DrawBoundingBox(BoundingBox box, Color color);                                   // Draw bounding box (wires)\13\n\13\nvoid DrawBillboard(Camera camera, Texture2D texture, Vector3 position, float scale, Color tint);   // Draw a billboard texture\13\n\13\nvoid DrawBillboardRec(Camera camera, Texture2D texture, Rectangle source, Vector3 position, Vector2 size, Color tint); // Draw a billboard texture defined by source\13\n\13\nvoid DrawBillboardPro(Camera camera, Texture2D texture, Rectangle source, Vector3 position, Vector3 up, Vector2 size, Vector2 origin, float rotation, Color tint); // Draw a billboard texture defined by source and rotation\13\n\13\nvoid UploadMesh(Mesh *mesh, bool dynamic);                                            // Upload mesh vertex data in GPU and provide VAO/VBO ids\13\n\13\nvoid UpdateMeshBuffer(Mesh mesh, int index, const void *data, int dataSize, int offset); // Update mesh vertex data in GPU for a specific buffer index\13\n\13\nvoid UnloadMesh(Mesh mesh);                                                           // Unload mesh data from CPU and GPU\13\n\13\nvoid DrawMesh(Mesh mesh, Material material, Matrix transform);                        // Draw a 3d mesh with material and transform\13\n\13\nvoid DrawMeshInstanced(Mesh mesh, Material material, const Matrix *transforms, int instances); // Draw multiple mesh instances with material and different transforms\13\n\13\nBoundingBox GetMeshBoundingBox(Mesh mesh);                                            // Compute mesh bounding box limits\13\n\13\nvoid GenMeshTangents(Mesh *mesh);                                                     // Compute mesh tangents\13\n\13\nbool ExportMesh(Mesh mesh, const char *fileName);                                     // Export mesh data to file, returns true on success\13\n\13\nbool ExportMeshAsCode(Mesh mesh, const char *fileName);                               // Export mesh as code file (.h) defining multiple arrays of vertex attributes\13\n\13\nMesh GenMeshPoly(int sides, float radius);                                            // Generate polygonal mesh\13\n\13\nMesh GenMeshPlane(float width, float length, int resX, int resZ);                     // Generate plane mesh (with subdivisions)\13\n\13\nMesh GenMeshCube(float width, float height, float length);                            // Generate cuboid mesh\13\n\13\nMesh GenMeshSphere(float radius, int rings, int slices);                              // Generate sphere mesh (standard sphere)\13\n\13\nMesh GenMeshHemiSphere(float radius, int rings, int slices);                          // Generate half-sphere mesh (no bottom cap)\13\n\13\nMesh GenMeshCylinder(float radius, float height, int slices);                         // Generate cylinder mesh\13\n\13\nMesh GenMeshCone(float radius, float height, int slices);                             // Generate cone/pyramid mesh\13\n\13\nMesh GenMeshTorus(float radius, float size, int radSeg, int sides);                   // Generate torus mesh\13\n\13\nMesh GenMeshKnot(float radius, float size, int radSeg, int sides);                    // Generate trefoil knot mesh\13\n\13\nMesh GenMeshHeightmap(Image heightmap, Vector3 size);                                 // Generate heightmap mesh from image data\13\n\13\nMesh GenMeshCubicmap(Image cubicmap, Vector3 cubeSize);                               // Generate cubes-based map mesh from image data\13\n\13\nMaterial *LoadMaterials(const char *fileName, int *materialCount);                    // Load materials from model file\13\n\13\nMaterial LoadMaterialDefault(void);                                                   // Load default material (Supports: DIFFUSE, SPECULAR, NORMAL maps)\13\n\13\nbool IsMaterialValid(Material material);                                              // Check if a material is valid (shader assigned, map textures loaded in GPU)\13\n\13\nvoid UnloadMaterial(Material material);                                               // Unload material from GPU memory (VRAM)\13\n\13\nvoid SetMaterialTexture(Material *material, int mapType, Texture2D texture);          // Set texture for a material map type (MATERIAL_MAP_DIFFUSE, MATERIAL_MAP_SPECULAR...)\13\n\13\nvoid SetModelMeshMaterial(Model *model, int meshId, int materialId);                  // Set material for a mesh\13\n\13\nModelAnimation *LoadModelAnimations(const char *fileName, int *animCount);            // Load model animations from file\13\n\13\nvoid UpdateModelAnimation(Model model, ModelAnimation anim, int frame);               // Update model animation pose (CPU)\13\n\13\nvoid UpdateModelAnimationBones(Model model, ModelAnimation anim, int frame);          // Update model animation mesh bone matrices (GPU skinning)\13\n\13\nvoid UnloadModelAnimation(ModelAnimation anim);                                       // Unload animation data\13\n\13\nvoid UnloadModelAnimations(ModelAnimation *animations, int animCount);                // Unload animation array data\13\n\13\nbool IsModelAnimationValid(Model model, ModelAnimation anim);                         // Check model animation skeleton match\13\n\13\nbool CheckCollisionSpheres(Vector3 center1, float radius1, Vector3 center2, float radius2);   // Check collision between two spheres\13\n\13\nbool CheckCollisionBoxes(BoundingBox box1, BoundingBox box2);                                 // Check collision between two bounding boxes\13\n\13\nbool CheckCollisionBoxSphere(BoundingBox box, Vector3 center, float radius);                  // Check collision between box and sphere\13\n\13\nRayCollision GetRayCollisionSphere(Ray ray, Vector3 center, float radius);                    // Get collision info between ray and sphere\13\n\13\nRayCollision GetRayCollisionBox(Ray ray, BoundingBox box);                                    // Get collision info between ray and box\13\n\13\nRayCollision GetRayCollisionMesh(Ray ray, Mesh mesh, Matrix transform);                       // Get collision info between ray and mesh\13\n\13\nRayCollision GetRayCollisionTriangle(Ray ray, Vector3 p1, Vector3 p2, Vector3 p3);            // Get collision info between ray and triangle\13\n\13\nRayCollision GetRayCollisionQuad(Ray ray, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4);    // Get collision info between ray and quad\13\n\13\n")
